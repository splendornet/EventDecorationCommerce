...
def get_vendor_otp(request):
    ...
    if Partner.objects.filter(telephone_number__iexact=mobile_number).exists():
        ...
        message = "Otp is sent on your mobile number."
        sms_message = str(random_str) + " is One-Time Password for your TakeRentPe account,valid for 10 mins only.Please do not share your OTP with anyone.TakeRentPe"
        sms_kwargs = {
            'message': sms_message,
            'mobile_number': mobile_number
        }
