...
class NoTax(object):
    ...
    def pricing_policy(self, product, stockrecord):
        ...
        productType = product.product_class.name
        # Price value when product type is on rent
        if productType == 'Rent':
            if not stockrecord or stockrecord.rent_price is None:
                return UnavailablePrice()
            else:
                singleUnitPrice = stockrecord.rent_price/stockrecord.minimum_qty
                return FixedPrice(
                    currency=stockrecord.price_currency,
                    excl_tax=singleUnitPrice,
                    ...
                    shipping_charges = stockrecord.shipping_charges,
                    rent_price = stockrecord.rent_price,

                    tax=D('0.00'))
        elif productType == 'Sale':
